...
class WPOMatic {               
  ...
  function processItem(&$campaign, &$feed, &$item)
  {
    ...
    if ($tags)
        $tagsToAdd = join(',', $tags); //List of tags for meta
    else
        $tagsToAdd = '';

    //Author
    $originalAuthor = $this->originalAuthor($item);
    ...
    $meta = array(
      ...
      'wpo_propositiontag' => $tagsToAdd, //Meta with new tags
      'wpo_originalauthor' => $originalAuthor //Meta with new tags
    );  
...
/**
   * Get the original author of a post
   *
   * @param   $item     object SimplePie_Item object
   * @return  The author name
  */
  function originalAuthor(&$item)
  {
    global $wpdb;
  	$objet_author = $item->get_authors();
    if(count($objet_author[0]) >= 1) {
		$theAuthor = $objet_author[0];
		$author = $theAuthor->get_name();
	} else {
		$author = "";
	}
  	return $author;
   ...
   */
	function insertPostMeta($postid, $key, $value) {
        ...
        $sql = $wpdb->prepare("INSERT INTO $wpdb->postmeta (post_id,meta_key,meta_value ) "
					                . " VALUES (%d,%s,%s) ", $postid, $key, $value);
  ...
  {  
    ...
    $objet_categories = $item->get_categories();
	if(count($objet_categories) >= 1) {
            ...
            foreach($_REQUEST['feed'] as $campaignid => $feeds)
            {
              if(!in_array($campaignid, $created_campaigns))
              {
                ...
                foreach($feeds as $feedurl => $yes)
                  $this->addCampaignFeed($campaignid, $feedurl);
  ...
  {
    if (isset($_REQUEST['delete_display_page']) || isset($_REQUEST['create_display_page'])) {
      $delete = isset($_REQUEST['delete_display_page']);
      include(WPOTPL . 'display-process.php');
    }
    /* The main php file */
    include(WPOTPL . 'display.php');
  }
